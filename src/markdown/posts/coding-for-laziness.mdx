...
不久我又刷到 <MagicLink username="antfu" text="Anthony Fu" /> 写的 [Categorize Your Dependencies](https://antfu.me/posts/categorize-deps)，里面提到了依赖分类的想法。我想遵循这个实践，不过手动把项目迁移到具名 `catalog:` 对我来说有亿点点累。
...
于是我写了 [pncat](https://github.com/jinghaihan/pncat)，做了几个命令：

- <code class="important-text-lime">detect</code>: 扫描项目，找出可以迁移到 catalog 的依赖
- <code class="important-text-purple">migrate</code>: 按规则自动分类并迁移
- <code class="important-text-orange">add/remove</code>: 安装或删除依赖时，自动更新 catalog
- <code class="important-text-cyan">clean</code>: 清理 catalog 里已不再使用的依赖
- <code class="important-text-amber">revert</code>: 把 catalog 配置还原成普通版本号

当然，分类没有标准答案。同一个依赖在不同类型的仓库中意图不同，往往适合不同的 `catalog:`，所以我实现了一套容易继承和扩展的规则系统。

```ts title="pncat.config.ts"
...
我通常用 [Raycast](https://www.raycast.com/) 插件打开项目，但我同时用 VS Code 和 Cursor，所以写了 [raycast-code-finder](https://github.com/jinghaihan/raycast-code-finder) 来合并 VS Code Base IDE 的历史打开记录。它也有 [VS Code](https://github.com/jinghaihan/vscode-crosside-code-finder) 插件版本。为了同步历史记录更方便，我还做了 [CLI](https://github.com/jinghaihan/code-finder) 版本，支持扫描指定目录并写入 IDE 的 SQLite 文件。
...
我的公司团队今年主要在做智能体方向，问答过程里的 Markdown 渲染有不少自定义要求。学习了 [streamdown](https://github.com/vercel/streamdown) 的 AST 解析方式后，我写了 [vue-stream-markdown](https://github.com/jinghaihan/vue-stream-markdown)。它支持流式渲染、代码高亮、Mermaid 图表，也能满足各种定制化刁难。

开发组件库时我碰到一个问题：[tsdown](https://github.com/rolldown/tsdown) 这类 bundler 不会自动给 SFC 写法的 Vue 组件引入自己的 CSS。通常只能手动把 CSS 合并到一个文件里，但这样开发体验不太好。我希望开发时继续用 SFC 写法，构建时再合并样式，所以把 Vite 的 CSS Code Splitting 搬到了 [tsdown](https://github.com/rolldown/tsdown/pull/654)。

## 最后

其实还有很多没列出来的[小玩具](/projects/contributions)。

感谢 <MagicLink username="antfu" text="Anthony Fu" /> 和 <MagicLink username="sxzz" text="Kevin Deng" /> 对我的一次又一次耐心指导。
