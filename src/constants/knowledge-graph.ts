...
const DOMAIN_DEFINITIONS: DomainDefinition[] = [
  ...
  },
  {
    ...
    kind: 'typescript',
    topics: [
      topic('ts-basic-types-inference', '基础类型与类型推断'),
      topic('ts-object-contracts', '对象建模与接口'),
      topic('ts-functions-overloads', '函数类型与重载'),
      topic('ts-generics', '泛型与泛型约束'),
      topic('ts-advanced-typing', '类型守卫与类型编程'),
      topic('ts-runtime-classes', '枚举、类与装饰器'),
      topic('ts-engineering-integration', '声明文件与工程集成'),
    ],
    backbone: [
  ...
  },
  {
    ...
    kind: 'nodejs',
    topics: [
      topic('nodejs-runtime-ecosystem', '运行时定位与 npm 生态'),
      topic('nodejs-event-loop-async', '事件循环与异步编程'),
      topic('nodejs-module-system', '模块系统与加载缓存'),
      topic('nodejs-binary-io', 'Buffer、Stream 与文件 I/O'),
      topic('nodejs-web-service', 'Express 与服务端通信'),
      topic('nodejs-service-security', '错误处理与服务安全'),
      topic('nodejs-data-cache', '数据库访问与缓存策略'),
      topic('nodejs-process-performance', '进程模型、并发与性能治理'),
      topic('nodejs-observability-delivery', '日志、测试与交付监控'),
    ],
    backbone: [
      'nodejs-runtime-ecosystem',
      'nodejs-event-loop-async',
      'nodejs-module-system',
      'nodejs-binary-io',
      'nodejs-web-service',
      'nodejs-service-security',
      'nodejs-data-cache',
      'nodejs-process-performance',
      'nodejs-observability-delivery',
    ],
  },
  {
    id: 'html-strategy',
    ...
    topics: [
      ...
      topic('html-form-controls', '表单与原生交互控件'),
  ...
  },
  {
    ...
    topics: [
      ...
      topic('origin-response', '源站返回响应'),
      topic('status-code-path', '状态码决策链'),
      topic('response-stream', '响应流接收'),
  ...
  {
    id: 'mini-program-architecture',
    title: '小程序体系',
    kind: 'miniapp',
    topics: [
      topic('mini-program-lifecycle', '应用与页面生命周期'),
      topic('mini-program-runtime-architecture', '双线程架构与 H5 差异'),
      topic('mini-program-data-binding', '数据绑定与 setData'),
      topic('mini-program-routing-navigation', '路由栈与页面跳转'),
      topic('mini-program-components-events', '组件化与事件系统'),
      topic('mini-program-performance-data-access', '性能优化、存储与网络请求'),
    ],
    backbone: [
      'mini-program-lifecycle',
      'mini-program-runtime-architecture',
      'mini-program-data-binding',
      'mini-program-routing-navigation',
      'mini-program-components-events',
      'mini-program-performance-data-access',
    ],
  },
  {
    id: 'vue-architecture',
...
const MAINLINE_EDGES: WorkflowEdge[] = [
  ...
  { source: 'request-lifecycle', target: 'origin-response', label: '源站开始返回状态行、响应头与响应体', kind: 'spine' },
  { source: 'origin-response', target: 'status-code-path', label: '响应状态码决定后续处理分支', kind: 'spine' },
  { source: 'status-code-path', target: 'response-stream', label: '状态判定完成后进入响应流消费阶段', kind: 'spine' },
  { source: 'response-stream', target: 'html-parser', label: 'HTML 响应流被解析器持续消费', kind: 'spine' },
